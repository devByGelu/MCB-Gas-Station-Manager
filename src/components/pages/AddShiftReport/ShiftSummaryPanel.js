...
import { openForm, changeActiveTabNav } from "../../../actions"

const ShiftSummaryPanel = ({ activeForm, openForm,changeActiveTabNav }) => {
        ...
        <List style={{ width: "100%" }}>
          {itemTexts.map((text, index) => (
            <ListItem key={index} role={undefined} dense>
              <ListItemIcon
                style={
                  isSettled(index) ? { color: theme.palette.primary.main } : {}
                }>
                {icons[index]}
              </ListItemIcon>
              <ListItemText
              ...
              />
              {item(isSettled(index), activeForm[indicators[index]], index)}
            </ListItem>
          ))}
        </List>
      </Grid>
